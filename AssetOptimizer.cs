...
{
    /// <summary>Utility to generate lower-resolution copies of assets (e.g. for faster loading or smaller builds).</summary>
    public static class AssetOptimizer
    {
        /// <summary>Copies PNG/JPG from sourceDir to targetDir: backgrounds resized to 1280px wide, sprites to 50%.</summary>
        public static void CreateLowResAssets(string sourceDir, string targetDir)
        {
            ...
            foreach (string file in Directory.GetFiles(sourceDir, "*.*", SearchOption.AllDirectories))
            {
                ...
                {
                    // 1280px wide for wide images (backgrounds), 50% for smaller sprites

    ...
    public partial class QuizForm
    {
        private void BtnSubmit_Click(object? sender, EventArgs e)
        {
            ...
            if (result.IsCorrect)
            {
                ...
                if (_monsterHealth <= 0) { XpSystem.AddXp(_settings.PlayerProgress, 50); ShowVictoryScreen(); }
                else { XpSystem.AddXp(_settings.PlayerProgress, 10); GenerateQuestion(); }
            }
            else
            {
        ...
        private void AnimateMeleeStrike(int damage)
        {
            ...
            _animationTimer.Tick += (s, e) => {
                ...
                else { _animationTimer.Stop(); _isAnimating = false; _picKnight.Location = _knightOriginalPos; UpdateMonsterSprite("idle"); UpdatePlayerStats(); LayoutCombat(); }
        ...
        private void AnimateMonsterAttack(int damage)
        {
            ...
            // Calculate distance to move toward the knight (Lunge Left)
            ...
            _animationTimer.Tick += (s, e) => {
                ...
                if (step <= 4) // Monster lunges forward (to the left)
                {
                    _picMonster.Left -= (distanceToKnight / 4);
                }
                else if (step == 5) // Impact
                {
                    ShowDamagePopup(_picKnight, damage, Color.Red);
                    _picKnight.Top += 10; // Flinch
                }
                else if (step <= 12) // Monster returns to original position
                {

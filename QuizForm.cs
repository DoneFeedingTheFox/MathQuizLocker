...
{
	/// <summary>
	/// Main game window acting as a secure "Lock Screen".
	/// Manages combat mechanics, independent UI scaling, and auto-updates via Velopack.
	/// </summary>
	public class QuizForm : Form
	{
		...
		private PictureBox _picMultiply = null!; // Multiplication symbol graphic
		...
		protected override CreateParams CreateParams
		{
			get
			{
				...
				cp.Style &= ~0x00080000; // WS_SYSMENU - Prevents right-click taskbar closing
				return cp;
			}
		}
		...
		{
			base.OnShown(e);
			_ = UpdateMyApp(); // Background check for v1.5.5+
			SpawnMonster();
			GenerateQuestion();
			UpdatePlayerStats();
			LayoutCombat();
		...
		private void InitializeCombatUi()
		{
			...
			this.Text = $"MathQuizLocker - v{assemblyVersion?.Major}.{assemblyVersion?.Minor}.{assemblyVersion?.Build}";

			// Security: Force focus back if user attempts to alt-tab
			this.Deactivate += (s, e) => {
			...
			this.KeyDown += (s, e) => {
				...
				{
					e.Handled = true;
					e.SuppressKeyPress = true; // Silences the "Pling"
			...
			_picMultiply = new PictureBox { SizeMode = PictureBoxSizeMode.Zoom, BackColor = Color.Transparent };

			// Load the multiplication graphic
			try
			{
				string xPath = AssetPaths.Dice("multiply.png");
				if (File.Exists(xPath)) _picMultiply.Image = Image.FromFile(xPath);
			}
			catch { }
		...
		private void LayoutCombat()
		{
			...
			_picMultiply.Visible = _die1.Visible;
